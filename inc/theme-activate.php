...
/**
 * Runtime safeguard (sin plugins):
 *
 * El hook de activación puede no ejecutarse en migraciones o si el tema se
 * actualiza sobre una base de datos existente. Si faltan las páginas de
 * sistema (login/registro/perfil) o no tienen el template asignado, las URLs
 * /es/registro, /en/register, etc. pueden terminar en 404 y mostrar páginas
 * "vacías".
 *
 * Esta rutina es idempotente: solo crea/ajusta lo necesario.
 */
function fcsd_ensure_system_pages_runtime(): void {
    if ( is_admin() ) return;
    ...
    foreach ( $system as $key => $def ) {
        ...
        // Si ya existe una página con ese template, la reutilizamos.
        $pid = fcsd_get_page_id_by_template($template);
        if ( ! $pid ) {
            $pid = fcsd_ensure_page($slug_ca, $title_ca, $template);
        }
        if ( ! $pid ) continue;
        ...
        fcsd_apply_page_i18n_meta((int)$pid, [
            '_fcsd_i18n_slug_es'   => $slug_es,
            '_fcsd_i18n_slug_en'   => $slug_en,
            '_fcsd_i18n_title_es'  => $title_es,
            '_fcsd_i18n_title_en'  => $title_en,
...
add_action('init', 'fcsd_ensure_system_pages_runtime', 2);
